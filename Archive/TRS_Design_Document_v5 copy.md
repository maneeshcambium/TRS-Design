...
10. [Resilience & Failure Modes](#12-resilience--failure-modes)
11. [Scale & Capacity](#13-scale--capacity)
12. [Cost Analysis](#14-cost-analysis)
13. [Key Engineering Decisions](#15-key-engineering-decisions)
14. [Operational Prerequisites](#16-operational-prerequisites)
15. [MVP Scope & Future Enhancements](#17-mvp-scope--future-enhancements)
16. [Tech Stack Reference](#18-tech-stack-reference)
...
SELECT
    ...
    ....
    COUNT(*) FILTER (WHERE ss.overall_perf_level = 5)     AS pl10,
    AVG(ss.overall_scale_score)                  AS avg_scale_score,
    AVG(ss.overall_raw_score)                    AS avg_raw_score,
    AVG(ss.overall_percent_correct)              AS avg_percent_correct,
...
- **Current year only.** The MV covers only the active school year. Historical component aggregates at school/district scope are served from `report_cache` (populated by ClickHouse Gold); live Postgres computation of historical component aggregates is not supported.
...
SELECT
    ...
    ...
    COUNT(*) FILTER (WHERE scs.perf_level = 3)            AS pl_10
    NOW()                                                 AS refreshed_at
...
ENGINE = AggregatingMergeTree()
ORDER BY (tenant_id, school_year, school_id, testalias_id);

CREATE MATERIALIZED VIEW trs.bronze_to_school_gold_mv
TO trs.school_aggregates_gold AS
SELECT
    s.tenant_id, s.school_year, m.school_id, s.testalias_id,
...
GROUP BY school_id, testalias_id
ORDER BY school_id;
```
...
ENGINE = AggregatingMergeTree()
ORDER BY (tenant_id, school_year, district_id, testalias_id);
...
SELECT
    ...
    ..
    sumState(if(s.overall_perf_level = 4, s.sign, 0))  AS pl_10_count,
    avgState(s.overall_scale_score * s.sign)            AS avg_score,
    uniqState(s.student_id)                             AS student_count
FROM trs.student_scores_bronze AS s
INNER JOIN (
...
WHERE s.is_aggregate_eligible = 1
GROUP BY s.tenant_id, s.school_year, m.district_id, s.testalias_id;
```


### 7.5 Layer 3: Gold — Component Aggregates (Standards, Reporting Categories, Writing Dimensions)

Component scores are high-volume (up to **1.65 billion rows/year at Texas scale**). They follow the same Sign-Pair Bronze CDC pattern as overall scores, but replicate through a dedicated Bronze table and feed a separate Gold table.

**Why a separate Gold table for components?** `student_component_scores` contains standards, reporting categories, and writing dimensions — often 10–30 component rows per student opportunity. Mixing this stream into `school_aggregates_gold` would bloat the overall score cube. A dedicated table keeps scans efficient and lets each `component_type` be queried independently.

#### Bronze: `student_component_scores_bronze`

PeerDB replicates `student_component_scores` CDC events through the Sign-Pair Transformer into this table. `REPLICA IDENTITY FULL` must be set (see §16).
...
#### Gold: `component_aggregates_gold`

A single Gold table for all component types, ordered so that single-`component_type` scans are efficient. Partitioned and ordered by `(component_type, component_id)` within each school/test grain.

```sql
CREATE TABLE trs.component_aggregates_gold (
    tenant_id       String,
    school_year     UInt16,
    school_id       String,
    testalias_id   String,
    ...
    avg_scale_score AggregateFunction(avg,  Float32),
    pl_1_count      AggregateFunction(sum,  Int64),
    pl_2_count      AggregateFunction(sum,  Int64),
    pl_3_count      AggregateFunction(sum,  Int64),
    student_count   AggregateFunction(uniq, Int32)
...
ENGINE = AggregatingMergeTree()
ORDER BY (tenant_id, school_year, school_id, testalias_id, component_type, component_id);

CREATE MATERIALIZED VIEW trs.bronze_to_component_gold_mv
TO trs.component_aggregates_gold AS
SELECT
    s.tenant_id,
    s.school_year,
    m.school_id,
    ...
    s.component_type,
    s.component_id,
    avgState(s.scale_score * s.sign)                              AS avg_scale_score,
    sumState(toInt64(if(s.perf_level = 1, s.sign, 0)))            AS pl_1_count,
    sumState(toInt64(if(s.perf_level = 2, s.sign, 0)))            AS pl_2_count,
    sumState(toInt64(if(s.perf_level = 3, s.sign, 0)))            AS pl_3_count,
    uniqState(s.student_id)                                       AS student_count
FROM trs.student_component_scores_bronze AS s
INNER JOIN (
    SELECT DISTINCT tenant_id, school_id, student_id
    FROM trs.membership_school_mirror FINAL
    WHERE is_deleted = 0
...
WHERE s.is_aggregate_eligible = 1
GROUP BY
    s.tenant_id, s.school_year, m.school_id,
    s.testalias_id, s.component_type, s.component_id;
```

**Gold Component read query (school scope):**
```sql
SELECT
    component_id,
    component_type,
    avgMerge(avg_scale_score)  AS average_score,
    uniqMerge(student_count)   AS students_tested,
    sumMerge(pl_1_count)       AS pl_1,
    sumMerge(pl_2_count)       AS pl_2,
    sumMerge(pl_3_count)       AS pl_3
FROM trs.component_aggregates_gold
WHERE tenant_id      = ?
  AND school_year    = ?
  AND school_id      = ?
  AND testalias_id  = ?
  AND component_type = ?    -- e.g. 'STANDARD'
GROUP BY component_id, component_type
ORDER BY component_id;
```

> **District-level component aggregates** use the same `component_aggregates_gold` table by supplying all `school_id` values for the district and re-aggregating. There is no separate district component Gold table — school-level pre-aggregated rows are sufficiently granular for district roll-up.
...
| Feature | Bronze | Silver | Gold — School | Gold — District | Gold — Component |
|---|---|---|---|---|---|
| **Engine** | `VersionedCollapsingMergeTree` | `AggregatingMergeTree` | `AggregatingMergeTree` | `AggregatingMergeTree` | `AggregatingMergeTree` |
| **Granularity** | 1 row per CDC event | 1 row per student opportunity | 1 row per school / test | 1 row per district / test | 1 row per school / test / component |
| **Primary Question** | "What changed and when?" | "Latest score per student?" | "School average?" | "District PL breakdown?" | "School standard/RC aggregate?" |
| **API Query Type** | Not queried directly | Roster-level reads (future) | School dashboard | District dashboard | School/District standards report |
| **Rescore Handling** | Signs cancel on background merge | `argMax` keeps highest `date_scored` | `sign × score_value` cancels old value | `sign`-adjusted PL indicator columns | `sign × scale_score`; `sign`-adjusted PL sums |
| **Dictionary Lookup** | None | None | `membership_school_mirror FINAL (JOIN)` | `membership_district_mirror FINAL (JOIN)` | `membership_school_mirror FINAL (JOIN)` |
...
| **School — components** | `report_cache` | `component_aggregates_gold` `avgMerge` + `sumMerge` | `mv_school_standards_current` (current year); long-TTL cache entry (historical) | 15 min |
| **District — components** | `report_cache` | `component_aggregates_gold` school-id roll-up | **No live Postgres fallback** — return HTTP 503 if cache and ClickHouse both unavailable | 15 min |
...
|--------|--------|-----------|
| `embargo_until` for the `testalias_id` | `trs.test_configs` | 60 s per `(tenant_id, testalias_id)` |
